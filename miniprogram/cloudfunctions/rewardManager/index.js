...
async function usePhoneTime(openid, data) {
  try {
    ...
    // 更新已使用时长
    await db.collection('user_rewards').where({
      _openid: openid
    }).update({
      data: {
...
async function applyPenalty(openid, data) {
  try {
    ...
    if (missedDays === 1) {
      ...
      // 清空所有使用权
      const userReward = await getUserRewards(openid)
      if (userReward.success) {
        penaltyMinutes = userReward.data.availableMinutes
      }
      recoveryDays = 7
      penaltyType = 'severe_penalty'
      penaltyTitle = '⚠️ 严重惩罚'
      penaltyMessage = `连续忘记打卡${missedDays}天，手机被回收7天，清空所有使用权`
    }
